...
- `GET /reports/platform-observability` ahora incluye tambien `promotion`, `cutover` y `operational_certification` con el gate del release actual
...
- `GET /reports/platform-observability` expone snapshot tecnico autenticado de plataforma, readiness de canales salientes, postura de `recovery`, `certification`, `promotion`, `cutover` y `operational_certification`
...
- el gate consolidado previo a produccion puede ejecutarse con `ops/scripts/check-production-go-no-go.sh`
...
- la plantilla espejo de `production` vive en `ops/github-environments/production.env.example`
- el bootstrap seguro de `production` vive en `ops/github-environments/production.variables.env.example`
