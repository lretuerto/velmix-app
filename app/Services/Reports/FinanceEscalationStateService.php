...
class FinanceEscalationStateService
{
    public function listByCode(int $tenantId): array
    {
        ...
        return DB::table('finance_escalation_states')
            ->leftJoin('users as acknowledged_users', 'acknowledged_users.id', '=', 'finance_escalation_states.acknowledged_by_user_id')
            ->leftJoin('users as resolved_users', 'resolved_users.id', '=', 'finance_escalation_states.resolved_by_user_id')
            ->where('finance_escalation_states.tenant_id', $tenantId)
            ->get([
                'finance_escalation_states.id',
                'finance_escalation_states.escalation_code',
                'finance_escalation_states.status',
                'finance_escalation_states.acknowledged_by_user_id',
                'finance_escalation_states.acknowledged_at',
                'finance_escalation_states.acknowledgement_note',
                'finance_escalation_states.resolved_by_user_id',
                'finance_escalation_states.resolved_at',
                'finance_escalation_states.resolution_note',
                'finance_escalation_states.last_seen_at',
                'finance_escalation_states.updated_at',
    ...
    private function upsertState(int $tenantId, int $userId, string $code, string $status, ?string $note): array
    {
        ...
        $existingId = DB::table('finance_escalation_states')
            ->where('tenant_id', $tenantId)
            ->where('escalation_code', $code)
            ->value('id');

        if ($existingId === null) {
            $existingId = DB::table('finance_escalation_states')->insertGetId([
                'tenant_id' => $tenantId,
            ...
            DB::table('finance_escalation_states')
                ->where('id', $existingId)
                ->update($updates);
        }
        ...
        app(TenantActivityLogService::class)->record(
            ...
            $userId,
            'finance',
            $status === 'acknowledged' ? 'finance.escalation.acknowledged' : 'finance.escalation.resolved',
            'finance_escalation_state',
            (int) $existingId,
            $status === 'acknowledged'
                ? sprintf('Finance escalation %s acknowledged.', $code)
                : sprintf('Finance escalation %s resolved.', $code),
            [
                'escalation_code' => $code,
                'status' => $status,
                'note' => $note,
