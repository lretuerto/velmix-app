...
class OperationsEscalationReportFlowTest extends TestCase
{
    ...
    public function test_reads_unified_operations_escalation_detail_for_finance_code(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertJsonPath('data.actions.acknowledge_path', '/reports/operations-escalations/finance/finance.stale_acknowledged/acknowledge')
            ->assertJsonPath('data.actions.resolve_path', '/reports/operations-escalations/finance/finance.stale_acknowledged/resolve')
            ->assertJsonPath('data.actions.source_acknowledge_path', '/reports/finance-escalations/finance.stale_acknowledged/acknowledge')
            ->assertJsonPath('data.actions.source_resolve_path', '/reports/finance-escalations/finance.stale_acknowledged/resolve');
    }

    public function test_admin_can_acknowledge_and_resolve_unified_operations_escalation(): void
    {
        $admin = $this->seedUserWithRole(10, 'ADMIN');
        $this->seedBillingScenario($admin);
        ...
        $this->actingAs($admin)
            ->withHeader('X-Tenant-Id', '10')
            ->postJson('/reports/operations-escalations/billing/billing.failed_backlog/acknowledge', [
                'note' => 'Tablero unificado en seguimiento.',
                'date' => '2026-03-12',
                'billing_days' => 3,
            ])
            ->assertOk()
            ->assertJsonPath('data.domain', 'billing')
            ->assertJsonPath('data.state.code', 'billing.failed_backlog')
            ->assertJsonPath('data.state.status', 'acknowledged')
            ->assertJsonPath('data.state.acknowledgement_note', 'Tablero unificado en seguimiento.')
            ->assertJsonPath('data.workflow_status', 'acknowledged');

        $this->actingAs($admin)
            ->withHeader('X-Tenant-Id', '10')
            ->postJson('/reports/operations-escalations/billing/billing.failed_backlog/resolve', [
                'note' => 'Caso cerrado desde la cola unificada.',
                'date' => '2026-03-12',
                'billing_days' => 3,
            ])
            ->assertOk()
            ->assertJsonPath('data.state.status', 'resolved')
            ->assertJsonPath('data.state.resolution_note', 'Caso cerrado desde la cola unificada.')
            ->assertJsonPath('data.workflow_status', 'resolved');
    ...
    public function test_cashier_cannot_manage_unified_operations_escalations(): void
    {
        $cashier = $this->seedUserWithRole(10, 'CAJERO');
